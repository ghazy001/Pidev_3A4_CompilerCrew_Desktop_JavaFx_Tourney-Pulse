            ...
            try (PreparedStatement selectJoueurStatement = connection.prepareStatement(selectJoueurSql)) {
                ...
                try (ResultSet joueurResultSet = selectJoueurStatement.executeQuery()) {
                    if (joueurResultSet.next()) {
                        ...
                        String updateAvisSql = "UPDATE AvisJoueur SET commaintre = ?, note = ?, idJoueur = ?, nom_joueur = ? WHERE idAvis = ?";

                        try (PreparedStatement updateAvisStatement = connection.prepareStatement(updateAvisSql)) {
                            updateAvisStatement.setString(1, avisJoueur.getCommentaire());
                            updateAvisStatement.setFloat(2, avisJoueur.getNote());
                            updateAvisStatement.setInt(3, idJoueur);
                            updateAvisStatement.setString(4, nomJoueur);
                            updateAvisStatement.setInt(5, id);

                            updateAvisStatement.executeUpdate();

                            System.out.println("Avis modifié avec succès.");
                        }
                    } else {
                        System.out.println("Aucun joueur trouvé avec l'ID " + avisJoueur.getIdJoueur());
                    }
                }
            }
        } catch (SQLException ex) {
